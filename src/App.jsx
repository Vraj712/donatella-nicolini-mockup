...
function App() {
  ...
  return (
    <div className="min-h-screen bg-luxury-dark text-luxury-cream font-sans overflow-x-hidden">
      ...
      {/* Navigation - Fixed for Tablet/Mobile/PC alignment */}
      <nav className="fixed w-full top-0 z-[60] flex justify-between items-center px-6 py-6 md:px-10 lg:px-20 mix-blend-difference">
        {/* Left Section */}
        <div className="text-[10px] tracking-[0.3em] font-light hidden sm:block w-1/3">
          MILAN / LONDON
        </div>

        {/* Center Section: Centered Logo Text */}
        <div className="text-lg md:text-xl lg:text-2xl font-serif tracking-[0.2em] text-center flex-grow whitespace-nowrap uppercase">
          Donatella Nicolini
        </div>

        {/* Right Section: Collections/Menu Button */}
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            <button onClick={() => setIsMenuOpen(false)} className="absolute top-8 right-8 text-luxury-gold cursor-pointer"><X size={32} strokeWidth={1}/></button>
      ...
      {/* Hero Section - Fixed Framing for Tablet and PC */}
      <section className="relative h-[100svh] w-full flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-6 w-full max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.5 }}>
            {/* Logo Image - Scaled for all devices */}
            <img 
              src="/logo.png" 
              className="w-16 h-16 sm:w-20 sm:h-20 md:w-28 md:h-28 mx-auto mb-6 md:mb-10 object-contain" 
              alt="Logo" 
            />
            
            <p className="text-[9px] md:text-xs tracking-[0.5em] uppercase mb-4 opacity-70">
              Fine Art Maternity
            </p>
            
            <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-9xl font-serif font-light mb-8 italic tracking-tight leading-tight">
              Ethereal <span className="not-italic text-luxury-gold">Grace</span>
            </h1>
            
            <button className="text-[10px] tracking-[0.3em] border-b border-luxury-gold/50 pb-2 hover:border-luxury-gold transition-all uppercase cursor-pointer">
              Explore the Gallery
            </button>
          </motion.div>
        </div>
          ...
          {['gallery-2.jpg', 'gallery-3.jpg', 'gallery-4.jpg', 'gallery-5.jpg', 'gallery-6.jpg', 'hero-bg.jpg'].map((img, i) => (
            ...
            >
              <div className="overflow-hidden mb-4 aspect-[3/4]">
                <img src={`/${img}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 opacity-80 group-hover:opacity-100" alt={`Gallery ${i}`} />
              </div>
      ...
      <footer className="bg-white text-black py-32 px-6 text-center">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
          ...
          <button className="px-12 py-5 bg-black text-white text-[10px] tracking-[0.4em] uppercase hover:bg-luxury-gold transition-colors cursor-pointer">
            Inquire Now
          </button>
        </motion.div>
      </footer>
